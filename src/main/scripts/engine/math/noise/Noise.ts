...
/**
 * Credit to vincismurf on the Unity forums for the original class, this one is modified a bit.
...
export default class Noise {
    ...
    public seed(seed?: number) {
        if(!seed)
            seed = Random.nextIntInRange(0, 65536);
        if (seed > 0 && seed < 1) {
            // Scale the seed out
            seed *= 65536;
        }
